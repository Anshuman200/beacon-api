import type { KeyValuePair } from "@/store/collectionStore";
import type {
  ScriptContext,
  ScriptRunResult,
  ScriptTestResult,
...
// This file runs inside a dedicated Web Worker (see scriptRunner.ts). Dedicated
// workers structurally have no `window`, `document`, cookies, or localStorage —
// that's what makes this a real sandbox boundary rather than a blocklist. We
// additionally neuter the worker's own network/import capabilities below so
// that even a full `new Function` sandbox escape (e.g. via the prototype chain)
// can't exfiltrate data over the network or pull in remote code.
// Some of these (e.g. `indexedDB`, `caches`) are getter-only accessor
// properties on WorkerGlobalScope — a plain `=` assignment throws in this
// module's strict mode. Object.defineProperty redefines them as plain
// (undefined) data properties instead.
function neuterGlobal(name: string) {
  try {
    Object.defineProperty(self, name, { value: undefined, writable: true, configurable: true });
  } catch {
    // Non-configurable in this engine — the dedicated-worker context itself
    // (no window/document/localStorage/cookies) remains the primary boundary.
  }
}

["fetch", "importScripts", "XMLHttpRequest", "WebSocket", "EventSource", "indexedDB", "caches"].forEach(
  neuterGlobal
);
...
function executeScript(code: string, context: ScriptContext): ScriptRunResult {
  const logs: string[] = [];
  ...
  // 9. Execute script. Identifier shadowing below is a first line of defense;
  // the real boundary is this being a dedicated worker with no window/document/
  // localStorage and neutered fetch/importScripts/WebSocket (see top of file).
...
// Executed code could itself call the real (unshadowed, escaped) postMessage
// to send arbitrary extra messages — tag our own result message so the main
// thread can't mistake one of those for the real, final result.
const RESULT_TAG = "beacon-script-result";

self.onmessage = (event: MessageEvent<ScriptWorkerRequest>) => {
  const { code, context } = event.data;
  const result = executeScript(code, context);
  self.postMessage({ tag: RESULT_TAG, result });
};
